...
const buildPrompt = ({ repoFullName, branch, filePath, score, endpointCount, smells }) => {
    ...
    return [
        ...
        '## Summary',
        '- 2 to 3 sentences. Mention the score and the biggest risk.',
        '',
        '## Priority fixes',
        '- List at most 3 fixes.',
        '- For each fix use this format:',
        '  - Severity + rule ID + rule name',
        '  - Affected endpoint(s) and line number(s) if available',
        '  - Why it matters',
        '  - Suggested change with a corrected endpoint example',
        '',
        '## Quick checklist',
